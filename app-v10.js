...
async function startWithLoading(){showScreen($('loadingScreen'));tone('start');await delay(700);state={version:10,module:draft.module||'fundamentos_oclusion',players:Array.from({length:draft.count},(_,i)=>({name:draft.names[i],position:0,skipTurns:0,character:draft.characters[i],color:CHARACTERS[draft.characters[i]].color})),current:0,usedQuestionIds:[],usedCaseIds:[],turn:1,locked:false};saveGame();showScreen($('game'));buildBoard();render();statusText.textContent=`${state.players[0].name}, tira los dos dados.`}
