...
function startCase(deck){const bank=activeCaseBank(),deckBank=bank.filter(x=>x.deck===deck),q=pickUnused(deckBank.length?deckBank:bank,'usedCaseIds');if(!q)return endTurn();pendingQuestion={...q,kind:'case'};selectedAnswer=null;tone('case');showQuestion()}
